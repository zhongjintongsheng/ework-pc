<template>
  <el-dialog title="新增字典" :visible.sync="dialog.visible" @closed="requestComplete" width="600px">
    <el-form :model="form" :rules="rules" ref="form" label-width="120px" class="rule-form" v-loading="dialog.loading">
      ...
      <el-form-item label="数据项编码：" prop="optionCode">
        <el-input v-model="form.optionCode" placeholder="请输入数据项名称" clearable class="input"></el-input>
      </el-form-item>
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        ...
        this.$http.post({
          url: this.$apis.dictList.addDict,
          param: this.form,
          loading: false,
          success: () => {
            this.$util.notify('新增成功')
