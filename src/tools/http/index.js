...
function download (opts = {}) {
  ...
  opts.success = (response) => {
    ...
    reader.onload = function (event) {
      ...
      util.notify(error.respMsg, 'error')
...
function getAll (opts = {}) {
  let { requests = [], loading = true } = opts
  let req = requests.map(({ url, param = undefined }) => {
    if (!url) {
  ...
  axios.all(req)
    .then(axios.spread((...response) => {
      for (let item of response) {
        let { respCode, respMsg } = item.data
        if (respCode !== config.SUCCESS_CODE) {
          util.notify(respMsg, 'error')
          fail(response)
          complete(response)
      ...
      let datas = response.map(({ data }) => data.data)
      success(datas)
      complete(datas)
    }))
    .catch(err => {
...
export default {
  ...
  /**
   * 批量get请求
   * requests: Array 如：[{ url: String, param: Object }] 由多个对象( 包含两个字段：url, param )组成的数组
   * success: Function 成功回调，参数为后台返回的 data
   * fail：Function 失败回调，参数为后台返回的 data
   * error：Function 异常回调，参数为 error 对象
   * complete：Function 请求结束都会执行的回调，参数为后台返回的 data  或者 error 对象
